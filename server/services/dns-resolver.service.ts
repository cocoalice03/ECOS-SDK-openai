...
export class DNSResolverService {
      ...
      try {
        ...
        for (const ip of this.BACKUP_IPS) {
          try {
            ...
            console.log(`❌ Backup IP ${ip} failed:`, (ipError as Error).message);
          }
        }
  ...
  static buildConnectionUrl(resolvedHost: string): string {
    // Utiliser la variable d'environnement ou extraire de DATABASE_URL
    const dbUrl = process.env.DATABASE_URL;
    if (dbUrl) {
      // Extraire les informations de connexion de DATABASE_URL
      const url = new URL(dbUrl);
      return `postgresql://${url.username}:${url.password}@${resolvedHost}:5432${url.pathname}`;
    }
