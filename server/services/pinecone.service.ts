...
export class PineconeService {
  ...
  constructor() {
    try {
      ...
      this.indexName = process.env.PINECONE_INDEX_NAME || "infirmierbeaujon";
  ...
  public async getEmbedding(
    ...
    try {
      ...
      // If dimensions are specified and different from default, set them
      if (dimensions && dimensions !== 1536) {
        modelConfig.dimensions = dimensions;
        console.log(`🔧 Using custom embedding dimension: ${dimensions}`);
      }

      const response = await openai.embeddings.create(modelConfig);
  ...
  async getAllSources(): Promise<string[]> {
    try {
      ...
      // Get index dimension dynamically
      let indexDimension = 1536; // Default
      try {
        const indexStats = await this.index.describeIndexStats();
        indexDimension = indexStats.dimension || 1536;
        console.log(
          `📏 Using index dimension for getAllSources: ${indexDimension}`,
        );
      } catch (error) {
        console.warn(
          "⚠️ Could not get index stats for getAllSources, using known dimensions...",
        );

        // For indexes like "infirmierbeaujon" that use embed models, we need to use their dimension
        if (this.indexName === "infirmierbeaujon") {
          indexDimension = 1024;
          console.log(
            `📏 Using known dimension for ${this.indexName}: ${indexDimension}`,
          );
        }
      }

      // Query with empty vector to get random samples and extract sources
      const dummyVector = new Array(indexDimension).fill(0);
      const queryResponse = await this.index.query({
        vector: dummyVector,
        topK: 100,
        includeMetadata: true,
  ...
  async deleteDocument(documentTitle: string): Promise<void> {
    try {
      ...
      if (idsToDelete.length > 0) {
        ...
        console.log(
          `Deleted ${idsToDelete.length} vectors for document: ${documentTitle}`,
        );
      }
    } catch (error) {
  ...
  async createIndex(
    ...
    try {
      console.log(
        `Attempting to create Pinecone index: ${indexName} with dimension: ${dimension}`,
      );
      ...
      if (error.message && error.message.includes("ALREADY_EXISTS")) {
        throw new Error(
          `L'index "${indexName}" existe déjà. Veuillez choisir un nom différent ou attendre quelques minutes si vous venez de le supprimer.`,
        );
      } else if (error.status === 403) {
        throw new Error(
          "Permission denied. Please check your Pinecone API key.",
        );
  ...
  async listIndexes(): Promise<
    Array<{ name: string; status?: string; dimension?: number }>
  > {
    ...
    try {
      ...
      console.log(
        "✅ Pinecone API response received:",
      ...
      console.log(
        "✅ Successfully processed indexes:",
  ...
  async processPDFContent(
    ...
    try {
      // Get index info to determine dimensions
      let indexDimension = 1536; // Default
      try {
        const indexStats = await this.index.describeIndexStats();
        indexDimension = indexStats.dimension || 1536;
        console.log(
          `📏 Current index "${this.indexName}" dimension: ${indexDimension}`,
        );
      } catch (error) {
        console.warn(
          "⚠️ Could not get index stats, checking if index uses embeddings model...",
        );
      ...
      // Create embeddings for all chunks with correct dimensions
      const embeddings = await this.getEmbeddingsForChunks(
        chunks,
        indexDimension,
      );
      ...
      const vectors = chunks.map((chunk, index) => ({
        id: `${title}_chunk_${index}_${Date.now()}`,
   ...
   */
  private async getEmbeddingsForChunks(
    chunks: string[],
    dimensions?: number,
  ): Promise<number[][]> {
    const embeddings = [];
    ...
    for (let i = 0; i < chunks.length; i += batchSize) {
      ...
      const batchEmbeddings = await Promise.all(
        batch.map((chunk) => this.getEmbedding(chunk, dimensions)),
      );
  ...
  async queryVectors(
    ...
    try {
      ...
      console.log(`🔍 Querying Pinecone index: "${indexName}" with query: "${query.substring(0, 100)}..."`);

      // Get embedding for the question with correct dimensions
      let embedding;
      if (indexName === "infirmierbeaujon") {
        embedding = await this.getEmbedding(query, 1024);
        console.log(`📏 Using 1024 dimensions for index: ${indexName}`);
      } else {
        embedding = await this.getEmbedding(query);
        console.log(`📏 Using default 1536 dimensions for index: ${indexName}`);
      }
      ...
      console.log(`📊 Pinecone query response: ${queryResponse.matches.length} matches found`);
      
      if (queryResponse.matches.length > 0) {
        console.log(`🎯 First match score: ${queryResponse.matches[0].score}`);
        console.log(`📄 First match metadata keys: ${queryResponse.matches[0].metadata ? Object.keys(queryResponse.matches[0].metadata) : 'none'}`);
      }
      ...
      for (const match of queryResponse.matches) {
        if (match.metadata && typeof match.metadata.text === "string") {
          ...
          console.log(`✅ Added result with ${match.metadata.text.length} characters`);
        } else {
          console.log(`⚠️ Skipping match - no text metadata found:`, match.metadata ? Object.keys(match.metadata) : 'no metadata');
        }
      }
