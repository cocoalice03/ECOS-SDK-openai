...
function ScenarioCreationForm({ email, onSuccess, editingScenario, onCancelEdit }: ScenarioCreationFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    duration: editingScenario?.duration || 20
  });
  ...
  const createScenarioMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setFormData({ title: "", description: "", patientPrompt: "", evaluationCriteria: "", pineconeIndex: "", criteriaText: "", duration: 20 });
    ...
    onError: (error) => {
      ...
      if (error.message && error.message.includes('pattern')) {
        alert("Erreur : Format de données invalide. Vérifiez que les critères d'évaluation sont au format JSON correct.");
      } else if (error.message && error.message.includes('JSON')) {
        alert("Erreur : Les critères d'évaluation doivent être au format JSON valide.");
      } else {
        alert(`Erreur lors de la ${editingScenario ? 'modification' : 'création'} du scénario. Détails: ${error.message || 'Erreur inconnue'}`);
      }
    }
  });
  ...
  const handleCreateScenario = () => {
    ...
    if (formData.evaluationCriteria && formData.evaluationCriteria.trim()) {
      try {
        const parsedCriteria = JSON.parse(formData.evaluationCriteria);
        // Valider que c'est un objet ou un tableau et non null
        if (typeof parsedCriteria === 'object' && parsedCriteria !== null) {
          criteria = parsedCriteria;
        } else {
          alert("Erreur : Les critères d'évaluation doivent être au format JSON valide. Accepte objet ou tableau avec label, description et weight.");
          return;
        }
      } catch (error) {
        alert("Erreur : Les critères d'évaluation doivent être au format JSON valide. Exemple : {\"anamnese\": 20, \"examen_physique\": 30} ou tableau avec label, description et weight.");
        return;
      }
    }
    ...
    createScenarioMutation.mutate({
      ...
      patientPrompt: formData.patientPrompt || null,
      evaluationCriteria: criteria,
      pineconeIndex: formData.pineconeIndex || null,
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">
          Décrivez précisément la situation clinique que l'étudiant devra gérer.
            ...
            {/* Duration Field */}
      <div>
        <Label htmlFor="duration">Durée du Scénario (minutes)</Label>
        <Input
          id="duration"
          type="number"
        ...
        />
        <p className="text-xs text-gray-500 mt-1">
          Indiquez la durée estimée de ce scénario en minutes.
        </p>
      </div>
...
Format détaillé:
{
  "evaluation_criteria": [
    {
      "label": "Communication adaptée",
      "description": "Évalue la qualité de la communication",
      "weight": 20
    }
  ]
}`}
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
            <div className="min-w-0">
              <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1 sm:mb-2">Tableau de bord Enseignant ECOS</h1>
              <p className="text-blue-100 text-base sm:text-lg">Gérez vos scénarios et suivez les progrès de vos étudiants</p>
              {email && <p className="text-xs sm:text-sm text-blue-200 mt-1 sm:mt-2 font-medium truncate">Connecté en tant que: {email}</p>}
            </div>
            <div className="flex items-center gap-4">
              <Badge variant="outline" className="bg-white/10 text-white border-white/20 backdrop-blur-sm px-3 sm:px-4 py-1 sm:py-2 text-xs sm:text-sm font-medium">
                Mode Enseignant
              </Badge>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 sm:py-6">
        <section className="hero bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl mb-4 sm:mb-8 border border-blue-100 shadow-sm">
          <div className="px-3 sm:px-6 py-6 sm:py-12">
            <div className="hero-content flex flex-col lg:flex-row lg:items-center gap-6 sm:gap-12">
              <div className="flex-1 order-2 lg:order-1">
                <div className="hero-text">
                  <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3 sm:mb-4">Plateforme Pédagogique Avancée</h2>
                          ...
                          <div className="flex gap-2 mt-4">
                            <Button 
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                window.open(`/student/${encodeURIComponent(email || '')}?scenario=${scenario.id}`, '_blank');
                              }}
                              className="flex-1"
                            >
