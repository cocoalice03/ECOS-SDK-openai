...
export default function AdminPage() {
  ...
  const switchIndexMutation = useMutation({
    mutationFn: async (indexName: string) => {
      ...
      console.log('Switch index response headers:', response.headers.get('content-type'));

      const responseText = await response.text();
      console.log('Switch index raw response:', responseText);

      if (!response.ok) {
        console.error('Switch index error response:', responseText);
        throw new Error(`Failed to switch index: ${response.status} ${responseText}`);
      }
